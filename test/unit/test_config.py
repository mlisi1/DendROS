...
class TestBasicConfig:
    def test_node_in_color_map(self):
        color_map, _, _, _, _ = load_config(fixture_path('basic.yaml'))
    ...
    def test_label_in_tag_map(self):
        _, tag_map, _, _, _ = load_config(fixture_path('basic.yaml'))
        assert tag_map['talker'] == 'TALK'

    def test_defaults_color_mode(self):
        _, _, _, _, defaults = load_config(fixture_path('basic.yaml'))
    ...
    def test_mode_map_empty_when_no_group_override(self):
        _, _, mode_map, _, _ = load_config(fixture_path('basic.yaml'))
        assert mode_map == {}

    def test_style_map_empty_when_no_group_override(self):
        _, _, _, style_map, _ = load_config(fixture_path('basic.yaml'))
        assert style_map == {}
...
class TestFullLineMode:
    def test_color_mode_full_line(self):
        _, _, _, _, defaults = load_config(fixture_path('full_line_mode.yaml'))
        assert defaults.get('color_mode') == 'full_line'

    def test_node_still_resolved(self):
        color_map, _, _, _, _ = load_config(fixture_path('full_line_mode.yaml'))
        assert 'talker' in color_map
...
class TestHexColorsConfig:
    def test_plain_hex_resolved(self):
        color_map, _, _, _, _ = load_config(fixture_path('hex_colors.yaml'))
    ...
    def test_labels_present(self):
        _, tag_map, _, _, _ = load_config(fixture_path('hex_colors.yaml'))
...
class TestRawAnsiConfig:
    def test_raw_code_unchanged(self):
        color_map, _, _, _, _ = load_config(fixture_path('raw_ansi.yaml'))
        assert color_map['raw_node'] == '34;1'

    def test_label_present(self):
        _, tag_map, _, _, _ = load_config(fixture_path('raw_ansi.yaml'))
...
class TestModifiersConfig:
    def test_bold_red(self):
        color_map, _, _, _, _ = load_config(fixture_path('modifiers.yaml'))
...
class TestUnmatchedColorConfig:
    def test_known_node_in_map(self):
        color_map, _, _, _, _ = load_config(fixture_path('unmatched_color.yaml'))
        assert 'known_node' in color_map

    def test_unmatched_color_set_in_defaults(self):
        _, _, _, _, defaults = load_config(fixture_path('unmatched_color.yaml'))
...
class TestNoTagConfig:
    def test_show_group_tag_false(self):
        _, _, _, _, defaults = load_config(fixture_path('no_tag.yaml'))
...
class TestMultiGroupConfig:
    def test_all_six_nodes_present(self):
        color_map, _, _, _, _ = load_config(fixture_path('multi_group.yaml'))
        expected = {
    ...
    def test_group_labels(self):
        _, tag_map, _, _, _ = load_config(fixture_path('multi_group.yaml'))
...
class TestNamespaceConfig:
    def test_full_path_key_in_color_map(self):
        color_map, _, _, _, _ = load_config(fixture_path('namespace.yaml'))
    ...
    def test_label_present(self):
        _, tag_map, _, _, _ = load_config(fixture_path('namespace.yaml'))
...
class TestEmptyGroupsConfig:
    def test_color_map_empty(self):
        color_map, _, _, _, _ = load_config(fixture_path('empty_groups.yaml'))
        assert color_map == {}

    def test_tag_map_empty(self):
        _, tag_map, _, _, _ = load_config(fixture_path('empty_groups.yaml'))
        assert tag_map == {}

    def test_defaults_still_returned(self):
        _, _, _, _, defaults = load_config(fixture_path('empty_groups.yaml'))
        assert defaults.get('color_mode') == 'tag_only'
...
class TestPerGroupShowTag:
    def test_nav_group_keeps_label(self):
        _, tag_map, _, _, _ = load_config(fixture_path('per_group_show_tag.yaml'))
        assert tag_map['nav2_controller'] == 'NAV'
    ...
    def test_hardware_group_still_has_color(self):
        color_map, _, _, _, _ = load_config(fixture_path('per_group_show_tag.yaml'))
...
class TestPerGroupColorMode:
    def test_hardware_group_in_mode_map(self):
        _, _, mode_map, _, _ = load_config(fixture_path('per_group_color_mode.yaml'))
    ...
    def test_package_default_still_tag_only(self):
        _, _, _, _, defaults = load_config(fixture_path('per_group_color_mode.yaml'))
        assert defaults.get('color_mode') == 'tag_only'
...
class TestInvertedTagConfig:
    def test_hardware_group_in_style_map(self):
        _, _, _, style_map, _ = load_config(fixture_path('inverted_tag.yaml'))
    ...
    def test_package_default_tag_style_inverted(self):
        _, _, _, _, defaults = load_config(fixture_path('inverted_tag.yaml'))
        assert defaults.get('tag_style') == 'inverted'

    def test_nodes_still_in_color_map(self):
        color_map, _, _, _, _ = load_config(fixture_path('inverted_tag.yaml'))
        assert 'nav2_controller' in color_map
        assert 'robot_state_publisher' in color_map

    def test_style_map_empty_without_group_tag_style(self):
        _, _, _, style_map, _ = load_config(fixture_path('per_group_color_mode.yaml'))
        assert style_map == {}
...
class TestConfigEdgeCases:
    ...
    def test_no_groups_key_returns_empty_maps(self, tmp_path):
        ...
        color_map, tag_map, mode_map, style_map, _ = load_config(str(cfg))
        assert color_map == {}
        assert tag_map == {}
        assert mode_map == {}
        assert style_map == {}
    ...
    def test_node_missing_color_key(self, tmp_path):
        ...
        color_map, _, _, _, _ = load_config(str(cfg))
    ...
    def test_node_missing_label_defaults_to_empty(self, tmp_path):
        ...
        _, tag_map, _, _, _ = load_config(str(cfg))
    ...
    def test_empty_nodes_list_no_crash(self, tmp_path):
        ...
        color_map, tag_map, _, _, _ = load_config(str(cfg))
        assert color_map == {}
    ...
    def test_null_defaults_returns_empty_dict(self, tmp_path):
        ...
        _, _, _, _, defaults = load_config(str(cfg))
    ...
    def test_group_color_mode_populates_mode_map(self, tmp_path):
        ...
        _, _, mode_map, _, _ = load_config(str(cfg))
    ...
    def test_group_tag_style_inverted_populates_style_map(self, tmp_path):
        ...
        _, _, _, style_map, _ = load_config(str(cfg))
